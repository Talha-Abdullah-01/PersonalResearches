			...
			for (var i = 0; i<4; i++)
				 {
					...
					if (binCorrect && decCorrect)
					 {
						cjsElement = getCJSElement("TEXTBOX",{subType:"CLOSABLE",x:305.6,y:309.9,width:452.65,height:75.9,titleId:"ID_CORRECT_TITLE",bodyId:"ID_CORRECT_BODY"});
						_container.addChild(cjsElement);

					}
					 else if (!binCorrect && !decCorrect) 
					{
						cjsElement = getCJSElement("TEXTBOX",{subType:"CLOSABLE",x:305.6,y:309.9,width:452.65,height:75.9,titleId:"ID_INCORRECT_TITLE",bodyId:"ID_INCORRECT_BODY"});
						_container.addChild(cjsElement);
